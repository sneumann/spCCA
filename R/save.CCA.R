...
function(CCA3,filename){
    ...
    for (numcv in c(1:dim(CCA3$cc3.weight.x)[2])){
        ...
        lambdas<-paste("lambda.x:",CCA3$lambda[1,numcv],"lambda.y:",CCA3$lambda[2,numcv],"lambda.z:",CCA3$lambda[3,numcv])
        corr<-CCA3$corr[numcv]
        fname=paste(filename,"_",numcv,".txt",sep="")

	    write(file=fname,paste("Results spCCA, lambdas:",lambdas,"\n",paste(numcv,". Variable",sep=""),"correlation:",corr,"\n"),append=F)
